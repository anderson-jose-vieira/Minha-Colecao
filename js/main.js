...
var GlobalNames = ['Um Sonho de Liberdade', 'Matrix', 'O Senhor dos Anéis', 'Avatar', 'Titanic'];
  ...
  Objeto.addEventListener('keyup', function (event) {
    if (event.key === 'Enter') {
      ...
      if (ValorDigitado) {
		...
		localStorage.setItem('listaFilmes',GlobalNames)
      }
...
function ExibirVetor() {
  ...
  const meusFilmes = localStorage.getItem('listaFilmes');
  if(meusFilmes != null){
	  GlobalNames = meusFilmes.split(',');
...
ul.addEventListener('click', function (event) {
  ...
  if (event.target.localName === 'button') {
	...
	localStorage.setItem('listaFilmes',GlobalNames) //Recebendo os dados que estão no Vetor. Atualizando o Banco.
